...
cleanup_failed_deliverables() {
  python3 - <<PY "$STATE_FILE" "$ROOT_DIR/Deliverable"
import json, os, re, shutil, stat, subprocess, sys
from pathlib import Path

state_file = Path(sys.argv[1])
deliverable_root = Path(sys.argv[2])
try:
    state = json.loads(state_file.read_text(encoding="utf-8"))
except Exception:
    print("skipped_failed_deliverable_cleanup:state_unreadable")
    raise SystemExit(0)

names = []
for key in ("project_name", "resolved_project_name"):
    value = str(state.get(key) or "").strip()
    if value and re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]{0,127}", value) and value not in names:
        names.append(value)
...
def remove_with_docker_helper(path):
    ...
    try:
        ...
        if path.is_dir():
            try:
                ...
                failed.append(
                    f"{path.name}:{type(exc).__name__}:{exc};docker_helper={type(helper_exc).__name__}:{helper_exc}"
                )
        else:
...
if failed:
    ...
    print(prefix + ":failed=" + ";".join(failed))
elif removed:
    print("removed_failed_deliverables:" + ",".join(removed))
elif names:
    print("skipped_failed_deliverable_cleanup:no_matching_outputs")
else:
    print("skipped_failed_deliverable_cleanup:no_project_name")
PY
}
...
python3 "$ROOT_DIR/.codex/scripts/update_state.py" \
  ...
  --set portable_subagent_fallback_reason= \
	  --set delivery_mode= \
	  --set portable_final_required= \
	  --set result= \
  --set exit_code= \
	  --set target= \
	  --set docker_cleanup= \
	  --set deliverable_cleanup= \
	  --set project_name= \
...
PY
  ...
  CURRENT_AGENTS_DIR="$agents_dir"
	  ...
	  printf -v codex_prompt '%s\n' \
	    ...
	    "2. Mode: delivery_mode=portable-deliverable. Follow AGENTS.md in the current APDv1 root as the workflow source of truth." \
    ...
    "4. Update state at actual task start:" \
    "   TARGET_JSON=\$(cat <<'JSON'" \
    "$record" \
    "JSON" \
    ")" \
	    "   python3 .codex/scripts/update_state.py --status RUNNING --batch-id \"$RUN_ID\" --task-id \"$task_id\" --target-index \"$idx\" --target-total \"$TOTAL\" --target-json \"\$TARGET_JSON\" --message \"Agent started\" --set delivery_mode=portable-deliverable --set portable_final_required=true --set audit_gate_required=portable_final" \
    "5. At final completion, agent must set one terminal state:" \
    ...
    "8. Constraints: avoid broad recursive scans of Deliverable/, DP_LOGS/, runtime-data, and archived output trees during bootstrap; use targeted reads/searches." \
    "9. Final terminal write must be the last task-state write, then exit with rc=0."
  ...
  python3 "$ROOT_DIR/.codex/scripts/update_state.py" \
    ...
    --set primary_audit_subagent_mode= \
	    --set primary_audit_subagent_fallback_reason= \
	    --set audit_gate_required=portable_final \
	    --set audit_attempts= \
	    --set audit_verdict= \
	    --set audit_result_file= \
	    --set portable_subagent_mode= \
	    --set portable_subagent_fallback_reason= \
	    --set delivery_mode=portable-deliverable \
	    --set portable_final_required=true \
...
PY
  ...
  if [[ "$final_status" =~ ^(COMPLETED_SUCCESS|COMPLETED_CONDITIONAL_SUCCESS)$ ]]; then
    validation_info="$(
      python3 - <<PY "$STATE_FILE"
import json, sys
try:
    ...
    print("\t")
    raise SystemExit(0)
project = str(d.get("project_name") or d.get("resolved_project_name") or "").strip()
mode = str(d.get("delivery_mode") or "").strip()
print("\t".join([project, mode]))
PY
    )"
    IFS=$'\t' read -r validation_project validation_mode <<< "$validation_info"
    if [ "$validation_mode" = "portable-deliverable" ] || [ "$validation_mode" = "" ]; then
      validation_result_file="$task_dir/final_output_validation.json"
      if [ -z "$validation_project" ]; then
        printf '{"ok":false,"errors":["missing project_name in task state"],"warnings":[]}\n' > "$validation_result_file"
        validation_rc=1
      else
        set +e
        python3 "$ROOT_DIR/.codex/scripts/validate_final_outputs.py" \
          "$validation_project" \
          --root "$ROOT_DIR" \
          --terminal-status "$final_status" \
          --require-audit \
          --json > "$validation_result_file"
        validation_rc=$?
        set -e
      fi
      if [ "$validation_rc" -ne 0 ]; then
        if [ -n "$result_warning" ]; then
          result_warning="${result_warning},final_output_validation_failed"
        else
          result_warning="final_output_validation_failed"
        fi
        WARNING_COUNT=$((WARNING_COUNT + 1))
  ...
  if [ "$final_result" = "failed" ]; then
    deliverable_cleanup="$(cleanup_failed_deliverables)"
  fi
  ...
  if [ "$script_writes_status" -eq 1 ]; then
    python3 "$ROOT_DIR/.codex/scripts/update_state.py" \
      ...
      --set "deliverable_cleanup=$deliverable_cleanup" \
